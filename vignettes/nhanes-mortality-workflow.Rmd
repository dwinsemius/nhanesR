...
# Set a custom session-local cache path (CRAN-safe)
...
When combining data from multiple two-year cycles, `nhanes_survival_prep()`
...
- If both `1999-2000` and `2001-2002` are present and the matching 4-year
  weight column exists, those early cycles use `4-year weight * (2/n)`.
- All other cycles use `2-year weight * (1/n)`.
- If the matching 4-year weight is unavailable, all cycles fall back to
  `2-year weight * (1/n)`.

`n` is the number of pooled cycles. The original unscaled 2-year weight is
preserved in `survey_weight_2yr_raw`.

**Important:** do not divide `survey_weight` again after
`nhanes_survival_prep()` has already created it.

---

**Weight adjustment for pooled cycles** is already handled inside
`nhanes_survival_prep()` when `weight_var` is provided.
