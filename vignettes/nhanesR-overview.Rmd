...
Three options control nhanesR behaviour. The package sets defaults at load
...
| Function | Purpose | Leads to |
|----------|---------|----------|
| `nhanes_survival_prep()` | Remove ineligible participants (`ELIGSTAT != 1`), create `time` and `event` columns, optionally create `event_cause` for cause-specific mortality | Downstream `survival`/`survey` modelling |
...
| Function | Purpose |
|----------|---------|
| `nhanes_cache_dir()` | View or change the local cache directory; see the **Setup** section above for the options that govern caching behaviour |
